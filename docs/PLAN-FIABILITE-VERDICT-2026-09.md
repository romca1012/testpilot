...
| F17 | `_base_helpers.py::navigate_menu` (step « je navigue vers le menu Odoo ») | Ouvre `/web#action=menu`, qui n'existe pas sur Odoo 17.0 Community (« L'action 'menu' n'existe pas ») : le clic sur l'application expire | Trouvé par le banc du lot 04 (2026-09-24) : 8 des 10 `technical_error` de la mesure de génération 17.0 (I3 = 20 %). **Mini-lot F17 (2026-09-24), avant 07b-e** : `_ouvrir_grille_applications` (commutateur `.o_navbar_apps_menu button`, Community ; ancienne route d'abord pour Enterprise) — voir `docs/RAPPORT-F17-2026-09-25.md`. **À valider sur Sapian (Enterprise)** avant de le considérer clos |
| F18 | steps de lecture `le champ … est égal à` (constat SÉPARÉ de F13, non traité) (`browse(id).read()` d'odoorpc) | Sur une facture, Odoo 17 échoue à sérialiser la réponse avec le `context` qu'odoorpc ajoute (`keys must be str … not frozendict`) ; `execute_kw` sans contexte fonctionne | Trouvé par le banc (2026-09-24), 17.0 ; aussi dans un step écrit par l'agent (1 cas de la génération). Lot 08 |
| F19 | `_base_helpers.py::validation_error_notification` (sélecteur `.o_notification.border-danger`) | Odoo 18.0 n'a plus cette classe (barre `span.o_notification_bar.bg-danger`) : la notification n'est jamais vue | **Faux FAILED** mesuré par le banc sur 18.0 (I2 = 13,3 %, 2 cas). Lot 08 |
| F20 | code généré par l'agent : `context.odoo_host` (attribut inexistant du contexte Behave) | L'agent invente un attribut de `context` (`AttributeError: 'Context' object has no attribute 'odoo_host'`) au lieu de lire `context.odoo_url` : `technical_error` au premier passage | Constat séparé de la génération 17.0 du banc (2026-09-24, cas `droit_suppression`, 1 cas sur 15). Piste : lister les attributs de `context` réellement posés dans le prompt ou le lint. Non traité |
| F21 | `_base_helpers.py::navigate_menu` (goto sans garde de connexion) | Ouvrait `/web#action=menu` sur un navigateur non connecté (le Contexte « je suis authentifié » ne connecte que le RPC) : page de connexion, 23 s de timeout | Génération 17.0 du banc (2026-09-25) : 4 cas sur 15. **Corrigé** avec F17 (`connecter_le_navigateur_si_page_vide`, partagé avec `navigate`) — `docs/RAPPORT-F17-2026-09-25.md` |
| F22 | code généré par l'agent : connexion RPC d'un compte secondaire | L'agent invente le mot de passe de `banc_commercial` (`Wrong login ID or password`) : il ne connaît que `ODOO_USER` | Génération 17.0 du banc (2026-09-25), 1 cas sur 15. Non traité ; à relier à D8 et aux lots 07b / 08d (comptes multiples) |
| F23 | code généré par l'agent : gestes d'interface Odoo réécrits à la main | `fill("")` refusé sur many2one, boutons d'enregistrement inventés (« Enregistrer manuellement »), sélection many2one, ligne de commande/facture, `Locator.triple_click` inexistant | Génération 17.0 du banc (2026-09-25) : 6 cas sur 15, cause dominante après F17/F21. Non traité ; c'est le périmètre du lot 08b |
| F24 | code généré par l'agent : composition de scénario | Un `Alors` lit « cet enregistrement » sans qu'aucun step ne l'ait mis en contexte (`RuntimeError`) ; et (mesure précédente) une assertion sur un enregistrement jamais créé donne un `non_conforme` sur l'instance saine | Génération 17.0 du banc (2026-09-25), 1 à 2 cas. Non traité |
| F25 | `_base_helpers.py::navigate_menu` (aucun contrôle d'arrivée) | Après le dernier segment, rien ne prouve qu'on est au bon endroit : un item homonyme ou un choix du repli adaptatif peut atterrir sur une page valide mais DIFFÉRENTE sans erreur. Risque de faux PASSED si un `Alors` ne dépend pas de l'écran (comptage RPC seul, sans lecture de l'interface) | Revue `verdict-reviewer` de la PR n° 12 (2026-09-25), point 3. Atténué (pas résolu) par l'erreur d'ambiguïté de `_cible_segment_menu`. Piste : contrôle STRUCTUREL de destination (`menu_id` / `action` du fragment d'URL après le dernier segment), jamais le texte de l'agent (décision 0015). Non traité ; à relier aux lots 02/03 (assertions indépendantes de l'écran) |
...
- **Rappel opérationnel (2026-09-25) — tout plafond de coût d'une campagne de mesure est appliqué EN AMONT** : arrêt automatique
  AVANT le premier essai qui pourrait le dépasser (`_boucle_essais`, code de sortie 3), jamais seulement rapporté après coup ; un
  plafond est obligatoire en mode génération (`banc_mesure.py --plafond-cout`). Une campagne qui rencontre une erreur de crédits
  s'ARRÊTE immédiatement, l'essai n'est pas compté (un manque de crédits n'est pas un échec de l'agent), et la mesure est publiée
  « INCOMPLÈTE ». Limite assumée : le plafond se juge entre deux essais, un essai peut le dépasser d'au plus son propre coût. Une
  mesure a coûté 6,78 $ pour ~2 $ annoncés (mini-lot F17) : le plafond limite le pire cas d'UNE campagne, pas le nombre de
  relances — **annoncer le budget total avant une série de relances**.
...
| D5 | Confiance du verdict | Champ `confiance` ∈ {`nominale`, `auto_resolue`, `apres_retry`} sur le résultat ; `passed` non nominal affiché « Réussi — à confirmer » ; option de campagne **stricte** (sans résolution adaptative ni retry) | 05 | ☑ (validée le 2026-09-25) — **complément du 2026-09-25 : le mode strict NE RÉPARE JAMAIS** (voir ci-dessous) |
| D5 (complément) | Le mode strict et la réparation automatique | **Le mode strict désactive la réparation automatique** : aucune réécriture par LLM après un échec, quel que soit le scénario ni le budget de réparation (`REPAIR_BUDGET_DEFAULT > 0` par défaut). L'explication IA reste active : purement descriptive, elle n'affecte pas le verdict. | 05 | ☑ (ajouté le 2026-09-25 sur consigne du porteur) — **Justification** : l'esprit du mode strict est qu'AUCUNE intervention, humaine ou par modèle, n'a eu lieu entre la génération et le verdict — pas seulement « aucun appel pendant l'exécution du scénario final ». Une réparation réécrit le test avec un LLM : un vert obtenu après elle ne serait plus un vert strict. Test : `tests/test_confiance_persistance.py::test_falsifiable_un_echec_en_campagne_stricte_ne_declenche_jamais_de_reparation`. |
| D6 | Code propre à une instance client | Notion de **profil d'instance** : `behave_runtime/steps_library/<connecteur>/profils/<profil>.py`, sélectionné par un réglage du projet ; Sapian devient le premier profil | 06 | ☑ (validée le 2026-09-25 par le porteur : profil d'instance, Sapian premier profil) |
| D7 | Oracle backend du connecteur web | Réglage de projet optionnel `oracle` (HTTP JSON authentifié, ou SQL lecture seule) ; présent ⇒ `ground_truth = backend_verified` | 07 | ☑ (validée le 2026-09-25) — **périmètre : HTTP d'abord** ; l'oracle SQL en lecture seule est un sous-lot SÉPARÉ, plus tard (isolation des pilotes, refus de toute requête non `SELECT`) |
| D8 | Plusieurs comptes par projet (droits, changement d'utilisateur) | Table `project_account` (libellé, identifiant, secret chiffré via `store/secrets.py`, rôle métier) ; steps « en tant que "<libellé>" » | 07b, 08 | ☑ (validée le 2026-09-25) — **précisions** : (1) le compte déjà configuré sur le projet devient le compte « principal » ; (2) l'agent de génération ne voit JAMAIS un secret, seulement les libellés — règle STRUCTURELLE (le secret n'entre dans aucun contexte de génération), pas une simple absence d'observation, même niveau de garantie que D10 bis ; (3) seuls `admin` et `dev` créent ou modifient un compte ; (4) l'API ne renvoie jamais un secret en clair, même à un admin — test explicite |
