...
| F13 | `_base_helpers.py::navigate_menu` (step « je navigue vers le menu Odoo ») | Ouvre `/web#action=menu`, qui n'existe pas sur Odoo 17.0 Community (« L'action 'menu' n'existe pas ») : le clic sur l'application expire | Trouvé par le banc du lot 04 (2026-09-24) : 8 des 10 `technical_error` de la mesure de génération 17.0 (I3 = 20 %). Lot 08 |
| F14 | steps de lecture `le champ … est égal à` (`browse(id).read()` d'odoorpc) | Sur une facture, Odoo 17 échoue à sérialiser la réponse avec le `context` qu'odoorpc ajoute (`keys must be str … not frozendict`) ; `execute_kw` sans contexte fonctionne | Trouvé par le banc (2026-09-24), 17.0 ; aussi dans un step écrit par l'agent (1 cas de la génération). Lot 08 |
| F15 | `_base_helpers.py::validation_error_notification` (sélecteur `.o_notification.border-danger`) | Odoo 18.0 n'a plus cette classe (barre `span.o_notification_bar.bg-danger`) : la notification n'est jamais vue | **Faux FAILED** mesuré par le banc sur 18.0 (I2 = 13,3 %, 2 cas). Lot 08 |
