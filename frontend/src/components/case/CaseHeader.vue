...
defineProps<{ c: CaseSummary; canAutomate?: boolean; automating?: boolean; hasGherkin?: boolean
  canEdit?: boolean; canDelete?: boolean; prevId?: number | null; nextId?: number | null }>()
  ...
  <div>
    <div class="flex items-center gap-3 flex-wrap">
      ...
      <div class="flex items-center gap-1.5 ml-auto text-muted-foreground">
        ...
        <button v-if="canEdit && canAutomate" class="min-h-11 min-w-40 rounded-md border border-border px-3 py-1.5 text-sm flex items-center justify-center gap-1.5 whitespace-nowrap hover:border-primary/40 text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:opacity-60 disabled:cursor-wait"
                :disabled="automating" :aria-busy="automating"
                :title="hasGherkin ? 'Créer une nouvelle version du test technique à relire' : 'Générer le test technique à partir du contenu métier saisi'" @click="emit('automate')">
          <svg class="w-4 h-4 shrink-0" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"><path d="M12 3l1.9 4.6L18.5 9l-4.6 1.9L12 15l-1.9-4.1L5.5 9l4.6-1.4z"/></svg>
          {{ automating ? 'Génération…' : hasGherkin ? 'Régénérer' : 'Automatiser avec l\'IA' }}</button>
        <button v-if="canEdit" class="rounded-md border border-border px-3 py-1.5 text-sm flex items-center gap-1.5 hover:border-primary/40 text-primary" @click="emit('edit')">
        ...
        <button v-if="canDelete" class="rounded-md border border-border px-3 py-1.5 text-sm flex items-center gap-1.5 hover:border-destructive/40 hover:text-destructive text-muted-foreground" title="Supprimer ce cas" @click="emit('delete')">
