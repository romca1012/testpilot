  ...
  <div>
    <div class="flex items-center gap-3 flex-wrap">
      ...
      <div class="flex items-center gap-1.5 ml-auto text-muted-foreground">
        <button class="hover:text-foreground p-1 disabled:opacity-30 disabled:cursor-not-allowed" title="Cas précédent" :disabled="!prevId" @click="prevId && emit('go', prevId)"><svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 6l-6 6 6 6"/></svg></button>
        <button class="hover:text-foreground p-1 disabled:opacity-30 disabled:cursor-not-allowed" title="Cas suivant" :disabled="!nextId" @click="nextId && emit('go', nextId)"><svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 6l6 6-6 6"/></svg></button>
