"""Cas FIGÉ du banc de mesure (lot 04) — facture_validation. Écrit à la main, sans LLM : il tient lieu de version de
...
@given('une facture client brouillon de 50 euros est créée')
def step_facture(context):
    context.facture_id = _E(context)["account.move"].create({
        "move_type": "out_invoice", "partner_id": _client(context),
        "invoice_line_ids": [(0, 0, {"name": "Ligne banc", "quantity": 1, "price_unit": 50.0})]})


@when('je valide la facture')
def step_valider(context):
    # `execute_kw` SANS contexte : avec le `context` qu'odoorpc ajoute à un appel de méthode, Odoo 17 échoue à
    # sérialiser la réponse de `action_post` (`keys must be str … not frozendict`) alors que le serveur valide.
    context.odoo.execute_kw("account.move", "action_post", [[context.facture_id]])


@then('la facture est comptabilisée')
def step_comptabilisee(context):
    # Lecture au niveau de la CLASSE (`read([id], …)`) : `browse(id).read(…)` d'odoorpc envoie un `context` avec lequel
    # Odoo 17 échoue à sérialiser la réponse pour une facture (`keys must be str … not frozendict`) — constat du banc,
    # voir le rapport du lot 04 (le step de bibliothèque « le champ … est égal à » y est sensible).
    etat = _E(context)["account.move"].read([context.facture_id], ["state"])[0]["state"]
    constater(etat == "posted", f"La facture n'est pas comptabilisée (état : {etat}).")
