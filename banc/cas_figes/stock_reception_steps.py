"""Cas FIGÉ du banc de mesure (lot 04) — stock_reception. Écrit à la main, sans LLM : il tient lieu de version de
...
@given('une demande de prix de 3 unités est créée')
def step_demande(context):
    context.achat_id = _E(context)["purchase.order"].create({
        "partner_id": _client(context),
        "order_line": [(0, 0, {"product_id": _produit(context), "name": "Produit banc",
                               "product_qty": 3, "price_unit": 60.0})]})


@when('je confirme la demande de prix')
def step_confirmer(context):
    _E(context)["purchase.order"].browse(context.achat_id).button_confirm()


@then("une réception est liée à la commande d'achat")
def step_reception(context):
    receptions = _E(context)["purchase.order"].read([context.achat_id], ["picking_ids"])[0]["picking_ids"]
    constater(len(receptions) > 0, "Aucune réception n'est liée à la commande d'achat confirmée.")
