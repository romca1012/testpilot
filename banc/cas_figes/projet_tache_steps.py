"""Cas FIGÉ du banc de mesure (lot 04) — projet_tache. Écrit à la main, sans LLM : il tient lieu de version de
...
@given('un projet de test existe')
def step_projet(context):
    context.projet_id = _E(context)["project.project"].create({"name": "Projet banc " + context.tentative_token})


@when('je crée une tâche dans ce projet')
def step_tache(context):
    context.tache_id = _E(context)["project.task"].create(
        {"name": "Tâche banc", "project_id": context.projet_id})


@then('la tâche est rattachée au projet')
def step_rattachee(context):
    tache = _E(context)["project.task"].read([context.tache_id], ["project_id"])[0]
    constater(bool(tache["project_id"]) and tache["project_id"][0] == context.projet_id,
              "La tâche n'est pas rattachée au projet créé.")
