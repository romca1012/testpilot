"""Cas FIGÉ du banc de mesure (lot 04) — crm_opportunite. Écrit à la main, sans LLM : il tient lieu de version de
...
@when('je crée une opportunité de test')
def step_creer(context):
    context.opportunite_nom = "Opportunité banc " + context.tentative_token
    context.opportunite_id = _E(context)["crm.lead"].create(
        {"name": context.opportunite_nom, "type": "opportunity"})


@then("l'opportunité créée existe avec son nom")
def step_existe(context):
    lignes = _E(context)["crm.lead"].read([context.opportunite_id], ["name"])
    constater(bool(lignes) and lignes[0]["name"] == context.opportunite_nom,
              "L'opportunité créée est introuvable ou son nom diffère.")
