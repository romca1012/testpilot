"""Cas FIGÉ du banc de mesure (lot 04) — vente_livraison. Écrit à la main, sans LLM : il tient lieu de version de
...
@when('je confirme le devis')
def step_confirmer(context):
    _E(context)["sale.order"].browse(context.commande_id).action_confirm()


@then('un bon de livraison est lié à la commande de vente')
def step_livraison(context):
    livraisons = _E(context)["sale.order"].read([context.commande_id], ["picking_ids"])[0]["picking_ids"]
    constater(len(livraisons) > 0, "Aucun bon de livraison n'est lié à la commande confirmée.")
