...
def test_purger_un_cas_AVEC_un_vrai_resultat_au_registre_ne_PLANTE_PAS(conn):
    """⚠️ Bug réel, trouvé le 2026-08-07 sur une VRAIE purge de production (module « Retenue de
    garantie », dont les cas avaient été réellement exécutés) : `test_result` (migration 25,
    2026-08-06) et `run_case_assignment` référencent `test_case`/`execution` par FK, mais la
    cascade de `CaseRepo.purger` ne les nettoyait pas — `DELETE FROM test_case` échouait en
    `sqlite3.IntegrityError: FOREIGN KEY constraint failed`, et la purge (censée être la SEULE
    destruction réelle du produit) devenait impossible sur tout cas déjà exécuté dans une
    campagne. Même défaut déjà payé deux fois pour `cost_ledger` et `test_run_case` (voir les
    notes dans `CaseRepo.purger`) — un troisième oubli, même cause : une table ajoutée après coup,
    jamais raccordée à la cascade de purge."""
    ...
    ModuleRepo(conn).delete(mid, par="qa")
    ModuleRepo(conn).purger(mid)   # ⚠️ plantait ici avant le correctif
